...
async function scrape(target, keywords) {
    ...
    try {
        console.log(`\n--> [Custom-BocaRaton] Conectando a Boca Raton: ${target.url}`);
        ...
        for (const kw of keywordList) {
            ...
            if (valid.length > 0) {
                results.push(...valid);
                console.log(`    ✅ Boca Raton [${kw}]: ${valid.length} encontradas`);
            }
        }
    } catch (error) {
        console.error(`    ❌ Error en Boca Raton:`, error.message);
    } finally {
        await browser.close();
    }
    return results;
