...
async function scrape(target, keywords) {
    ...
    try {
        console.log(`\n--> [Custom-BrowardHealth] Conectando a Broward Health: ${target.url}`);
        ...
        for (const kw of keywordList) {
            ...
            if (valid.length > 0) {
                results.push(...valid);
                console.log(`    ✅ Broward Health [${kw}]: ${valid.length} encontradas`);
            }
        }
    } catch (error) {
        console.error(`    ❌ Error en Broward Health:`, error.message);
    } finally {
        await browser.close();
    }
    return results;
