...
async function scrape(target, keywords) {
    ...
    try {
        console.log(`\n--> [Custom-CoconutCreek] Conectando a Coconut Creek: ${target.url}`);
        ...
        for (const kw of keywordList) {
            ...
            if (valid.length > 0) {
                results.push(...valid);
                console.log(`    ✅ Coconut Creek [${kw}]: ${valid.length} encontradas`);
            }
        }
    } catch (error) {
        console.error(`    ❌ Error en Coconut Creek:`, error.message);
    } finally {
        await browser.close();
    }
    return results;
