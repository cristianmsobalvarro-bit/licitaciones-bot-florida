...
async function scrape(target, keywords) {
    ...
    try {
        console.log(`\n--> [Custom-PortEverglades] Conectando a Port Everglades: ${target.url}`);
        ...
        for (const kw of keywordList) {
            ...
            if (valid.length > 0) {
                results.push(...valid);
                console.log(`    ✅ Port Everglades [${kw}]: ${valid.length} encontradas`);
            }
        }
    } catch (error) {
        console.error(`    ❌ Error en Port Everglades:`, error.message);
    } finally {
        await browser.close();
    }
    return results;
