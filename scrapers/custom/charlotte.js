...
async function scrape(target, keywords) {
    ...
    try {
        console.log(`\n--> [Custom-Charlotte] Conectando a Charlotte County: ${target.url}`);
        ...
        const items = await page.evaluate((countyName) => {
            const data = [];
            const rows = document.querySelectorAll('table tr, .bid-row, .solicitation-item');
            rows.forEach(row => {
                ...
                if (titleEl) {
                    ...
                    data.push({
                        ...
                        fuente: 'Charlotte County Portal'
                    });
                }
            });
            return data;
        ...
        for (const kw of keywordList) {
            ...
            if (valid.length > 0) {
                results.push(...valid);
                console.log(`    ✅ Charlotte [${kw}]: ${valid.length} encontradas`);
            }
        }
    } catch (error) {
        console.error(`    ❌ Error en Charlotte:`, error.message);
    } finally {
        await browser.close();
    }
    return results;
