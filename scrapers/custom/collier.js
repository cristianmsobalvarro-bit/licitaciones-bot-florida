...
async function scrape(target, keywords) {
    ...
    try {
        console.log(`\n--> [Custom-Collier] Conectando a Collier County: ${target.url}`);
        ...
        const items = await page.evaluate((countyName) => {
            const data = [];
            const rows = document.querySelectorAll('table tr, .bid-row');
            rows.forEach(row => {
                ...
                if (titleEl) {
                    ...
                    const dateEl = row.querySelector('td:nth-child(3), td:last-child');
        ...
        for (const kw of keywordList) {
            ...
            if (valid.length > 0) {
                results.push(...valid);
                console.log(`    ✅ Collier [${kw}]: ${valid.length} encontradas`);
            }
        }
    } catch (error) {
        console.error(`    ❌ Error en Collier:`, error.message);
    } finally {
        await browser.close();
    }
    return results;
