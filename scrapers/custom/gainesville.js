...
async function scrape(target, keywords) {
    ...
    try {
        console.log(`\n--> [Custom-Gainesville] Conectando a Gainesville: ${target.url}`);
        ...
        for (const kw of keywordList) {
            ...
            if (valid.length > 0) {
                results.push(...valid);
                console.log(`    ✅ Gainesville [${kw}]: ${valid.length} encontradas`);
            }
        }
    } catch (error) {
        console.error(`    ❌ Error en Gainesville:`, error.message);
    } finally {
        await browser.close();
    }
    return results;
