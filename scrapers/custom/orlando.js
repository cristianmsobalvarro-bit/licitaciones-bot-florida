...
async function scrape(target, keywords) {
    ...
    try {
        console.log(`\n--> [Custom-Orlando] Conectando a Orlando: ${target.url}`);
        ...
        for (const kw of keywordList) {
            ...
            if (valid.length > 0) {
                results.push(...valid);
                console.log(`    ✅ Orlando [${kw}]: ${valid.length} encontradas`);
            }
        }
    } catch (error) {
        console.error(`    ❌ Error en Orlando:`, error.message);
    } finally {
        await browser.close();
    }
    return results;
