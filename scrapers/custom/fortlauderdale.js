...
async function scrape(target, keywords) {
    ...
    try {
        console.log(`\n--> [Custom-FortLauderdale] Conectando a Fort Lauderdale: ${target.url}`);
        ...
        for (const kw of keywordList) {
            ...
            if (valid.length > 0) {
                results.push(...valid);
                console.log(`    ✅ Fort Lauderdale [${kw}]: ${valid.length} encontradas`);
            }
        }
    } catch (error) {
        console.error(`    ❌ Error en Fort Lauderdale:`, error.message);
    } finally {
        await browser.close();
    }
    return results;
