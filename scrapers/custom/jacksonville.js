...
async function scrape(target, keywords) {
    ...
    try {
        console.log(`\n--> [Custom-Jax] Conectando a Jacksonville: ${target.url}`);
        ...
        for (const kw of keywordList) {
            ...
            if (valid.length > 0) {
                results.push(...valid);
                console.log(`    ✅ Jacksonville [${kw}]: ${valid.length} encontradas`);
            }
        }
    } catch (error) {
        console.error(`    ❌ Error en Jacksonville:`, error.message);
    } finally {
        await browser.close();
    }
    return results;
