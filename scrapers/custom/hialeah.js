...
async function scrape(target, keywords) {
    ...
    try {
        console.log(`\n--> [Custom-Hialeah] Conectando a Hialeah: ${target.url}`);
        ...
        for (const kw of keywordList) {
            ...
            if (valid.length > 0) {
                results.push(...valid);
                console.log(`    ✅ Hialeah [${kw}]: ${valid.length} encontradas`);
            }
        }
    } catch (error) {
        console.error(`    ❌ Error en Hialeah:`, error.message);
    } finally {
        await browser.close();
    }
    return results;
